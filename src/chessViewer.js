...
// 根据棋子名称获取对应的PNG图片base64编码
window.getChessPieceImage = function(pieceName) {
	if (!pieceName || typeof pieceName !== 'string') {
		return null;
	}

	return window.chessPieceImages[pieceName] || null;
};
		...
		try {
			...
			const board = Chessboard(boardId, {
				...
				pieceTheme: function(piece) {
					return window.getChessPieceImage(piece)
				}
			});
			...
			if (infoDiv) {
				infoDiv.innerHTML = '<span style="color: red;">PGN parse failed: ' + error.message + '</span>';
			}
		}
	});
}
